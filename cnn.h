...
public:
	...
	linear(int _fin, int _fout) {
		...
		//out = tensor<float>({BATCH_SIZE, fout});
		//grad_out = tensor<float>({BATCH_SIZE, fin});
	...
	tensor<float> forward(tensor<float>& _in) {
		...
		out = tensor<float>::matmul(in, weights);
		/*
...
class model {
public:
	vector<layer*> layers; // the last layer is the loss layer
	...
	float train_step(tensor<float>& input, tensor<float>& target) {
		...
		for (int i = layers.size() - 1; i >= 0; i--) {
			//cout << "backward layer " << i << ": " << grad << endl;
			layer* l = layers[i];
